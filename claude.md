...
- `aranet4` library (handles BLE communication) and `python-dotenv` (config)
...
- **aranet4 library**: Uses `aranet4.client.get_current_readings(mac)` instead of raw bleak. Handles BLE service discovery and data parsing internally.
...
## Config (.env)
- `ARANET_MAC` - Bluetooth MAC of Aranet4 (must be paired via `bluetoothctl pair`; "Smart Home integrations" enabled in Aranet Home app)
