...
- **Recover** (on the Pi): `pkill -f 'aranet_logger.py --single'` then `sudo systemctl restart bluetooth`. See `.cursor/skills/crontab-scheduling/SKILL.md` for the full pile-up story (`fcntl` lock + cron). The script runs D-Bus `Adapter1.StopDiscovery` and `bluetoothctl scan off` before scans and between retries. Set `BLUEZ_ADAPTER_DBUS_PATH` only to a path that exists on **this** machine (`busctl tree org.bluez` — usually `/org/bluez/hci0`). After `systemctl restart bluetooth`, the next scan runs in a **child Python process** (`--ble-recovery-scan`) so Bleak’s D-Bus client is not reused across the restart (that reuse often produced `InProgress` even on an idle adapter). After `InProgress`, `aranet_logger.py` retries in-process after clearing discovery / restarting bluetooth; only if all retries fail does `--single` exit with `BluezScanConflictError`.
