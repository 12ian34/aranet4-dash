...
- **Recover** (on the Pi): `pkill -f 'aranet_logger.py --single'` then `sudo systemctl restart bluetooth`. See `.cursor/skills/crontab-scheduling/SKILL.md` for the full pile-up story (`fcntl` lock + cron). After `InProgress`, `aranet_logger.py` restarts bluetooth then **exits** (`--single`) or backs off (daemon) so the **next** run uses a new process — retrying `find_nearby` in the same process usually hit `InProgress` again.
